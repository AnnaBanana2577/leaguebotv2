...
module.exports = async () => {
    ...
    var table = new AsciiTable3('Leaderboard')
    .setHeading('Rank', 'Name', 'Points');

    let count = 1;
    let playerStats = [];
    for (pName in names) {
    playerStats.push([count, AsciiTable3.truncateString(names[pName], 15), topPlayers[pName][1]])
    count ++;
    }
    ...
    lbMessage.edit(`Last Updated: <t:${epoch}:f>\n` + "```" + table.toString() + "```");
