...
module.exports = async (message) => {
  ...
  const isMember = message.member.roles.cache.some(
    (role) => role.name === "League Members"
  );

  if (!isMember) {
    message.channel.send(
      "Only League Members can join league matches. To become a member, use `!register`"
    );
    return;
  }
